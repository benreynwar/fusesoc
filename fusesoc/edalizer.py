...
from fusesoc import generation
...
def parse_generatorargs(parameter_infos, backendargs):
    """
    Extract generator arguments from the remaining command line arguments.
    """
    parser = argparse.ArgumentParser()
    for param_name, param_info in parameter_infos.items():
        if param_info.paramtype == 'generatorarg':
            parser.add_argument('--{}'.format(param_name), type=TYPE_MAP[param_info.datatype])
    parser.add_argument('backendargs', nargs=argparse.REMAINDER)
    args = parser.parse_args(backendargs)
    top_parameters = {}
    for param_name, param_info in parameter_infos.items():
        if param_info.paramtype == 'generatorarg':
            top_parameters[param_name] = getattr(args, param_name)
    return top_parameters, args.backendargs
...
class Edalizer(object):
    ...
    def __init__(self, vlnv, flags, cores, cache_root, work_root, export_root=None,
        ...
        top_core = cores[-1]
        top_parameters, backendargs = parse_generatorargs(top_core.parameters, backendargs)

        top_node, core_info = generation.cores_to_nodes(cache_root, _flags, cores)
        top_node = generation.nodes_to_neighborhoods(top_node)
        ...
        cores = generation.get_new_cores(core_info)
